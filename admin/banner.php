...
<head>
    ...
    <title>Jungle Resort | Banner</title>
                                ...
                                <tbody>
                                    ...
                                    while ($row = $result->fetch_assoc()) {
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-center"><img src="upload/banner/<?php echo $row['img']; ?>"
                                            ...
                                            <td class=text-center>
                                                ...
                                                <button type="button" name="update3" class="btn btn-primary btn-sm m-2"
                                                    onclick="myfcn1(<?php echo $row['id']; ?>,'<?php echo $row['descc']; ?>','<?php echo $row['img']; ?>')"
                                                    data-toggle="modal" data-target="#updatebanner" title="Edit"
                                                    aria-hidden="true">
                                                    <i class='fas fa-edit'></i>
                                                </button>
                                                <a onclick="confirmDelete(<?php echo $row['id']; ?>, tb='banner', tbc='id', returnpage='banner.php');"
                                                    title="Delete">
                ...
                <div class="modal-body">
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="form-group col-12">
                                <label for="exampleInputcname">Banner Description:</label>
                                <input type="text" class="form-control" name="banner_desc" required>
                            </div>
                        </div>
                    </div>
                </div>
...
if (isset($_POST['banner_insert'])) {
    ...
    $sql = "INSERT INTO banner (descc, img, status) VALUES ('$banner_desc', '$new_file_name', '1')";
        ...
        <div class="modal-content">
            ...
            <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" enctype="multipart/form-data">
                <div class="modal-body p-4">
                    <input type="text" name="id1" id="id11">
                    <div class="form-group">
                        <label for="text">Banner Name:</label>
                        <input type="text" class="form-control" id="descc1" name="descrip">
                    </div>
                    <div class="form-group col-12">
                        <label for="image">Banner Image:</label>
                        <input type="file" name="image" class="form-control" placeholder="image"
                            accept=".png, .jpg, .jpeg"
                            onchange="document.getElementById('image112').src = window.URL.createObjectURL(this.files[0])">
                        <img id="image112" src="dist/img/noimage1.png" alt="New image" width="50" height="50" />
                        <img src="#" id="image11" alt="Profile Image" width="50" height="50" class="mt-2 img-fluid">
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-danger" data-dismiss="modal">Close</button>
...
if (isset($_POST['banner_update'])) {
    ...
    if ($image_name == NULL) {
        ...
        $sql1 = "UPDATE banner SET descc='$descrip', updated_at='$currentDateTime' WHERE id='$id'";
        ...
        $sql_previous_image = "SELECT img FROM banner WHERE id='$_POST[id1]'";
        ...
        if (move_uploaded_file($image_tmp, $target_file)) {
            ...
            $sql1 = "UPDATE banner SET descc='$descrip', img='$new_file_name',updated_at='$currentDateTime' WHERE id='$id'";
            if ($conn->query($sql1) == true) {
                echo "<script>
                        $(document).ready(function(){
                        toastr.success('Form submitted successfully');
