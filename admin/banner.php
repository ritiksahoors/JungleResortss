                                        ...
                                        <tr>
                                            ...
                                            <td class=text-center>
                                                ...
                                                <button type="button" name="update3" class="btn btn-primary btn-sm m-2"
                                                    onclick="myfcn9(<?php echo $row['id']; ?>,'<?php echo $row['img']; ?>','<?php echo $row['descc']; ?>')"
...
if (isset($_POST['banner_insert'])) {
    ...
    $sql = "INSERT INTO banner (descc, img, status) VALUES ('$desc_nm', '$new_file_name', '1')";
            ...
            <form action="<?php $_SERVER['PHP_SELF']; ?>" method='post' enctype="multipart/form-data">
                <div class="modal-body p-4">
                    ...
                    <div class="form-group col-12">
                        ...
                        <img src="#" id="ban_img" alt="Profile Image" width="50" height="50" class="mt-2 img-fluid">
                    </div>
                    <div class="form-group">
                        <label for="text">Description:</label>
                        <input type="text" class="form-control" id="bann_desc" name="desc_nm">
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-danger" data-dismiss="modal">Close</button>
...
if (isset($_POST['banner_update'])) {
    ...
    if ($image_name == NULL) {
        ...
        $desc_nm = htmlspecialchars($_POST["desc_nm"], ENT_QUOTES, 'UTF-8');
        $currentDateTime = date('Y-m-d H:i:s');

        $sql1 = "UPDATE banner SET descc='$desc_nm', updated_at='$currentDateTime' WHERE id='$id'";
        ...
        $sql_previous_image = "SELECT img FROM banner WHERE id='$_POST[id9]'";
        $result = $conn->query($sql_previous_image);
        if ($result->num_rows > 0) {
            $row = $result->fetch_assoc();
            $previous_image = $upload_dir . $row['img'];
        ...
        if (move_uploaded_file($image_tmp, $target_file)) {
            ...
            $sql1 = "UPDATE banner SET descc='$desc_nm', img='$new_file_name', updated_at='$currentDateTime' WHERE id='$id'";
            if ($conn->query($sql1) == true) {
                echo "<script>
                        $(document).ready(function(){
                        toastr.success('Form submitted successfully');
